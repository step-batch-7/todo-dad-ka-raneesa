...
const createTodoList = function(req, res) {
  ...
  if (req.app.locals.todoLists[req.username].addTodo(title)) {
...
const serveTodoLists = function(req, res) {
  const todoList = req.app.locals.todoLists[req.username].todoList;
  writeTodoLists(JSON.stringify(req.app.locals.todoLists));
...
const removeTodoList = function(req, res) {
  const { id } = req.body;
  if (req.app.locals.todoLists[req.username].removeTodo(id)) {
    return serveTodoLists(req, res);
  }
  return serveNotFoundPage(req, res);
};
...
const addTask = function(req, res) {
  ...
  if (req.app.locals.todoLists[req.username].addTask(id, work)) {
    return serveTodoLists(req, res);
  }
  return serveNotFoundPage(req, res);
};
...
const removeTask = function(req, res) {
  ...
  if (req.app.locals.todoLists[req.username].removeTask(todoId, taskId)) {
    return serveTodoLists(req, res);
  }
  return serveNotFoundPage(req, res);
};
...
const changeStatusOfTask = function(req, res) {
  const { todoId, taskId } = req.body;
  if (req.app.locals.todoLists[req.username].changeStatus(todoId, taskId)) {
    return serveTodoLists(req, res);
  }
  return serveNotFoundPage(req, res);
};
...
const renameTodo = function(req, res) {
  ...
  if (req.app.locals.todoLists[req.username].renameTodo(newTitle, todoId)) {
    return serveTodoLists(req, res);
  }
  return serveNotFoundPage(req, res);
};
...
const renameTask = function(req, res) {
  ...
  if (req.app.locals.todoLists[req.username].renameTask(newTitle, todoId, taskId)) {
    return serveTodoLists(req, res);
  }
  return serveNotFoundPage(req, res);
};
...
const createNewUser = function(details, usersDetails, todoLists) {
  ...
  todoLists[details.username] = [];
  writeFile('../data/todoLists.json', JSON.stringify(req.app.locals.todoLists));
};
...
const login = function(req, res) {
  ...
  if (isValidUser(req.app.locals.usersDetails, details)) {
    ...
    req.app.locals.usersDetails.push({ name: 'bd' });
